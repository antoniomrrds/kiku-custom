...
async function renderKiku(): Promise<void> {
  if (!container.value || !host.value) return;
  ...
  let style = shadow.querySelector("#kiku-embed-style");
  if (!style) {
    style = document.createElement("style");
    style.id = "kiku-embed-style";
    style.textContent = kikuEmbedCss;
    shadow.appendChild(style);
  }
  const styleTags = [style].filter(Boolean) as HTMLStyleElement[];
  ...
  if (import.meta.env.DEV) {
    ...
    let link = shadow.querySelector('link[data-kiku-docs-style="true"]') as HTMLLinkElement | null;
    if (!link) {
  ...
  let root = shadow.querySelector("#kiku-root") as HTMLDivElement | null;
  if (!root) {
  ...
  const ankiFields: typeof exampleFields = {
    ...exampleFields,
    ...Object.fromEntries(
...
onMounted(async () => {
  ...
  const observer = new MutationObserver(() => {
    queueMicrotask(() => {
      ...
      const root = shadow?.querySelector("#kiku-root");
      if (!root) return;

      const isRootDarkMode = root.hasAttribute("data-dark-mode");
      if (isDark !== isRootDarkMode) {
        if (isDark) root.setAttribute("data-dark-mode", "");
        else root.removeAttribute("data-dark-mode");
      }
    });
