...
function KikuIcon() {
  const [$$latestVersion] = createResource(async () => {
    ...
    const checked = sessionStorage.getItem(
  ...
  return (
    <div class="flex flex-col items-center text-base-content-faint justify-center">
      ...
      <div class="flex items-center gap-1.5">
        <KikuVersion
          latestVersion={
            $$latestVersion.state === "ready" ? $$latestVersion() : null
          }
        />
      </div>
    </div>
...
function KikuVersion(props: { latestVersion?: string | null }) {
  const $version = createMemo(() =>
    props.latestVersion && props.latestVersion !== constants.VERSION
      ? props.latestVersion
      : null,
  );
  return (
...
function Section(props: { children: JSX.Element }) {
  return (
    <div class="flex flex-col gap-2 sm:gap-4 animate-fade-in relative">
      {props.children}
    </div>
  );
}
...
function GeneralSettings() {
  const { $isConfigOutOfSync, $setConfig, $config } = useConfigContext();
          ...
          options={[
            {
              ...
              description:
                "Shows Selection, Main Definition, and Glossary as separate pages.",
            },
            {
              value: "single-page",
              label: "Single Page (Appended)",
              description:
                "Appends all definitions into a single scrollable page.",
            },
            {
  ...
  return (
    <Section>
      <div class="flex gap-x-4 sm:gap-x-2 gap-y-1 items-center flex-wrap">
        ...
        <div
          role="tablist"
          class="tabs tabs-sm sm:tabs-md tabs-box self-start flex-nowrap"
        >
          <button
            role="tab"
            class="tab"
        ...
        <Show when={$hasModified() && $darkMode() !== initialDarkMode}>
          <div class="text-xs text-base-content-faint flex items-center gap-2">
            <span>
              {$darkMode() ? "Dark" : "Light"} theme has been emulated
            </span>
            <button
...
function ThemeGrid(props: {
  selected: DaisyUITheme;
  onSelect: (theme: DaisyUITheme) => void;
}) {
  return (
...
function FontSizeRangeSetting(props: {
  ...
  return (
    <Section>
      ...
      <Show when={isAnkiDroidNewStudyScreen}>
        <div role="alert" class="alert alert-warning">
          AnkiDroid integration is not available on AnkiDroid new study screen
          yet.
        </div>
      </Show>
      <div>
        <div class="grid grid-cols-[repeat(auto-fit,minmax(10rem,1fr))] rounded-box gap-x-4 gap-y-2">
          <ToggleSetting
            configKey="ankiDroidEnableIntegration"
    ...
    <div class="collapse rounded-none collapse-arrow">
      ...
      <div class="collapse-content p-0">
        <div class="flex flex-col gap-4 animate-fade-in ">
          <div class="grid grid-cols-[repeat(auto-fit,minmax(10rem,1fr))] rounded-box gap-x-4 gap-y-2">
            <TextSetting
              configKey="ankiConnectAddress"
              label="AnkiConnect Address"
            />
            <ToggleSetting
              configKey="showStartupTime"
              label="Show Startup Time"
            />
          </div>
          ...
          <div class="flex flex-col gap-2">
            <div class="flex gap-2 items-center">
              ...
              <ClipboardCopyButton
                text={() => JSON.stringify(initialAnkiFields, null, 2)}
              />
            </div>
...
function $KikuFiles() {
  ...
  const [$$kikuResource] = createResource(
    ...
    async () => {
      ...
      const missing = constants.IMPORTANT_FILES.filter(
